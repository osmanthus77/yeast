...
for my $gene ( sort keys %{$gene_info_of} ) {
    ...
    # position of the paired base (stack, O(n))
    my $pair_pos_of = {};
    my @stack;

    for my $i ( 0 .. $info->{length} - 1 ) {
        my $pos  = $i + 1;
        my $char = substr $info->{fold}, $i, 1;

        if ( $char eq '(' ) {
            push @stack, $pos;
        }
        elsif ( $char eq ')' ) {
            my $open_pos = pop @stack or next;
            $pair_pos_of->{$open_pos} = $pos;
            $pair_pos_of->{$pos}      = $open_pos;
        }
        else {
...
# gene
{
    ...
    print {$out_fh} join( "\t", "gene", "length", @heads ), "\n";
    for my $gene ( sort keys %{$gene_info_of} ) {
        my $info = $gene_info_of->{$gene};    # shortcut
        ...
        print {$out_fh} join "\t", $gene, $info->{length};
...
# vars
{
    ...
    my @heads = qw{ name fold_length gene_base gene_pos pair_base};
